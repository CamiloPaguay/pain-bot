...
export async function handleModoIlegal(m, conn) {
  if (!m.isGroup || !isModeActive('modoIlegal', m.chat) || !m.text || m.fromMe) return
  if (isModeActive('modoHuman', m.chat) || isModeActive('modoSad', m.chat) || isModeActive('modoPsico', m.chat)) return
