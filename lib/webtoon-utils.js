...
export async function searchWebtoons(query, language = 'es') {
  const url = `https://api.delirius.online/search/webtoons?query=${encodeURIComponent(query)}&language=${encodeURIComponent(language)}`
...
export async function fetchWebtoonInfo(listUrl) {
  ...
  const api = `https://api.delirius.online/download/webtoon?url=${encodeURIComponent(listUrl)}`
