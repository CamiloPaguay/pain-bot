...
import { handleAntiPalabra } from './Antis/anti-palabra.js'
...
export async function handleAntiSystems(m, conn, isAdmin, isOwner, isRAdmin, isBotAdmin, isPrems, commandExecuted) {
  ...
  const results = await Promise.allSettled([
    ...
    handleAntiPalabra(m, conn, isAdmin, rcanal),
    handleSoloAdmin(m, conn, isAdmin, isOwner, rcanal),
    handleCommandSuggestions(m, conn, commandExecuted, isAdmin, isOwner)
  ])
