...
export async function handleCommandSuggestions(m, conn, commandExecuted, isAdmin = false, isOwner = false) {
  if (m.text && !commandExecuted && !m.commandExecuted) {
    ...
    // .grupo off → bot apagado: sin sugerencias para no-admin/no-owner
